...
function InputRoot({ hasError = false, className, ...rest }: InputRootProps) {
	return (
		<div
			data-error={hasError}
			className={cn(
				'group bg-gray-800 h-12 border border-gray-600 rounded-xl px-4 flex items-center gap-3 focus-within:border-gray-100 not-focus-within:data-[error=true]:border-danger',
				className
			)}
			{...rest}
		/>
